...
from project.deep_rl.utils.misc import plot_from_monitor_results


def load_policy_class(policy_name):
    mod = importlib.import_module("project.deep_rl.policies")
    policy_class = getattr(mod, policy_name)
    return policy_class


def load_wrapper_class(wrapper_name):
    mod = importlib.import_module("project.deep_rl.utils.wrappers")
